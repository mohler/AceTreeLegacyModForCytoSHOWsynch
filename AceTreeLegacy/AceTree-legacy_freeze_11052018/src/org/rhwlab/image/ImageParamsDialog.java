...
import org.rhwlab.image.ImageWindowAT.ColorSchemeUI;
...
public class ImageParamsDialog extends JDialog implements ActionListener {
	...
	public ImageParamsDialog(ImageWindowAT imgWin) {
        super(imgWin, "Image Parameters", false);
        iImgWin = imgWin;

        Border blackline = BorderFactory.createLineBorder(Color.black);
        //iDispProps = imgWin.getDisplayProps();
        iCSUI = new ColorSchemeUI[iImgWin.iDispProps.length];
        iPanel = new JPanel();
        ...
        JPanel [] testPanel = new JPanel[iImgWin.iDispProps.length];
        ...
        //iImgWin.cAcbTree = false;
        //dummyPanel.add(iAcbTree);
        //dummyPanel.setBorder(blackline);
     
		/*
		JPanel optionPanel = new JPanel();
	        for (int i=0; i < iImgWin.iDispProps.length; i++) {
    ...
    public ImageParamsDialog(ImageWindowATstatic imageWindowATstatic) {
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if (command.equals("Reset")) {
            iImgWin.iDispProps = iImgWin.getDisplayProps();                
            for (int i=0; i < iImgWin.iDispProps.length; i++) {
                println("Reset: " + i + CS + iImgWin.iDispProps[i].iName
                        + CS + iImgWin.iDispProps[i].iLineageNum);
                iCSUI[i].iLabel.setText(iImgWin.iDispProps[i].iName);
                iCSUI[i].iCB.setSelectedIndex(iImgWin.iDispProps[i].iLineageNum);
            }
            ...
            for (int i=0; i < iImgWin.iDispProps.length; i++) {
                ...
                iImgWin.iDispProps[i].iName = name;
                iImgWin.iDispProps[i].iLineageNum = num;
            }
