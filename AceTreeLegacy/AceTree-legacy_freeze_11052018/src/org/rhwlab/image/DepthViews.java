...
public class DepthViews extends JPanel implements ActionListener {
    ...
    public DepthViews(String s) {
        ...
        iNucleiMgr = iAceTree.getNucleiMgr();        iImgWin = iAceTree.getiImgWin();
        makeUI();
    ...
    private void makeUI() {
        ...
        model = new SpinnerNumberModel(250, 10, iImgWin.cImageWidth, 10);
    ...
    private void yzView(int time, int x) {
        ...
        println("yzView, " + iplus.getWidth() + CS + iplus.getHeight() + CS + iImgWin.cImageHeight + CS + iNucleiMgr.getPlaneEnd());
	...
	int factor=(int)Math.round(iAceTree.getiZPixRes());
        ...
        ByteProcessor bp = new ByteProcessor(factor * planeEnd - 1, iImgWin.cImageHeight);
	...
	int factor=(int)Math.round(iAceTree.getiZPixRes());
        int planeEnd = iNucleiMgr.getPlaneEnd();
        ByteProcessor bp = new ByteProcessor(iImgWin.cImageWidth,factor * planeEnd - 1);
        for (int m=1; m <= planeEnd; m++) {
            ImageProcessor ip = getImage(time, m);
            if (ip == null) break;
    ...
    private ImageProcessor getImage(int time, int plane) {
        String imageFile = iImgWin.cZipTifFilePath;
        imageFile += "/" + iImgWin.cTifPrefix;
    ...
    private ImageProcessor getRedData(String greenName) {
        //System.out.println("getRedData: " + greenName + C.CS + iImgWin.cUseZip);
        if (iImgWin.cUseZip == 2) return getRedZipData(greenName);
        FileInputStream fis;
        ImagePlus ip = null;
        //String ss = "/home/biowolp/AncesTree/temp2/images/050405-t050-p15.tif";
        try {
            fis = new FileInputStream(greenName);
            byte [] ba = iImgWin.readByteArray(fis);
            ip = iImgWin.openTiff(new ByteArrayInputStream(ba), false);
            fis.close();
	    if(iImgWin.imagewindowUseStack==1) {
		int markerChannel=1;
		ip=iImgWin.splitImage(ip,markerChannel);
	    }
            //ip = readData(fis);
        } catch(IOException ioe) {
            //System.out.println("iImgWin.test3 exception ");
