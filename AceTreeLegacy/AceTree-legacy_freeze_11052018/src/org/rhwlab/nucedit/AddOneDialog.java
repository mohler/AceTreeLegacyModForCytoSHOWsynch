...
// 	     JDialog iDialog = new EIDialog1(iAceTree, iAceTree.getImageWindowAT(), false, c, time);
	}else if( o==iKillDeep){
...
//                 iAdjust.setSelected(true);
            ...
            Nucleus n = iImgWin.cNucleiMgr.getNucleusFromHashkey(iCurrentCell.getHashKey(), iImageTime + iTimeInc);
            if(n==null)
            	return;
    ...
    private void checkTimesAndPropogateValue(String field){
    	...
    	for (int i=starttime;i<=endtime;i++){
    		n = iImgWin.cNucleiMgr.getNucleusFromHashkey(iCurrentCell.getHashKey(), i);
...
//         } else 
	    if (button == 3) {
            updateCurrentInfo(false);
            Nucleus n = iImgWin.cNucleiMgr.findClosestNucleus(e.getX(), e.getY(), iImagePlane + iPlaneInc, iImageTime + iTimeInc);
            if (n == null) {
...
//         }
    ...
    protected void addCell(int x, int y, boolean continuation) {
        ...
        updateCurrentInfo(false);
        int time = iImageTime + iTimeInc;
        Vector nuclei = iImgWin.cNucleiMgr.getNucleiRecord().elementAt(time - 1);
...
//        iAceTree.setCurrentCell(c, iImageTime + iTimeInc, AceTree.RIGHTCLICKONEDITIMAGE);
    ...
    protected void updateCurrentInfo(boolean detectChange) {
        ...
        Vector nuclei = iImgWin.cNucleiMgr.getNuclei(iImageTime + iTimeInc - 1);
