...
import org.rhwlab.image.ImageWindowAT;
import org.rhwlab.image.ImageWindowATstatic;
...
//    private EditImage   iEditImage2;
	...
	private int iSplitChannelImage;
    //private GeneralStartupError iGSE;
	private ImagePlus sourceImp;
	private boolean imagePlusSourceOfImages =false;

	private VTree iVTree;
	...
	public AceTree(String configFileName, boolean fullGUI) {
        super();
        ...
        Config c = Config.createConfigFromXMLFile(configFileName);
        ...
        iSplitChannelImage = c.iSplitChannelImage;
        ImageWindowATstatic.setStaticParameters(iZipTifFilePath,
                iTifPrefix, iUseZip,
                iSplitChannelImage, iSplit);
        ImageWindowATstatic.imagewindowPlaneNumber = 1;
        int k = bringUpSeriesData(shortName);
        this.iImgWin = new ImageWindowAT(configFileName, ImageWindowATstatic.makeImage(iTifPrefix+  "t1.tif"), new PlayerControl(this));
...
//        this.iRootEstablished = false;
        ...
        displayTree();
        try {
	        iTrackPosition = ImageWindowAT.ANTERIOR;
    ...
    @SuppressWarnings("static-access")
	public AceTree(ImagePlus imp, String nucZipPath, boolean fullGUI) {
        super();
        imagePlusSourceOfImages  = true;
        sourceImp = imp;
        String shortZipName = Config.getShortName(nucZipPath);
    	iSeriesLabel=new JLabel("AceTree");
        this.iMainFrame= new JFrame(TITLE);
        iNucleiMgr = new NucleiMgr(nucZipPath);
        this.iNucleiMgrHash = new Hashtable<String, NucleiMgr>();
        this.iImgWin = new ImageWindowAT(imp, new PlayerControl(this));        
        int k = bringUpSeriesData(imp, nucZipPath);
        iNucleiMgr = iNucleiMgrHash.get(shortZipName);
        iNucleiMgr.getConfig().iZipNucDir = "nuclei/";
        iAncesTree = iNucleiMgr.getAncesTree();
        iCellsByName = iNucleiMgr.getCellsByName();
        this.iImgWin = new ImageWindowAT(imp.getTitle(), imp, new PlayerControl(this));
        iImgWin.setAceTree(this);
        iImgWin.cNucleiMgr = iNucleiMgr;
        AceTree.fullGUI=fullGUI;
//		System.out.println("string constructor");
...
//    }
    	...
    	if (configFileName != null) {
    		...
    		if (configFileName != null) {
    			if(configFileName.equals("-full")){
					...
					iAceTree.setConfigFileName(configFileName);
    			}	
    		}
    	} else if (iAceTree == null){
    ...
    public synchronized static AceTree getAceTree(ImagePlus imp, String nucZipPath) {
		AceTree iAceTree = null;

    	if (imp != null) {
    		iAceTree = new AceTree(imp, nucZipPath, false);
    	} else {
    		iAceTree = new AceTree(null,false);
    	}
...
//     }
    ...
    public void bringUpSeriesUI(String configFileName) {
    	...
    	try {
	    	// Reset ImageWindowAT use stack flag
    		newLine();
	        System.out.println("ImageWindowAT stack flag reset to 0 in AceTree.");
	        if (iImgWin == null){
	        	iImgWin = new ImageWindowAT();
	        }
//	        iImgWin.setUseStack(0);
	        ...
	        iNucleiMgr.sendStaticParametersToImageWindowAT();
    ...
    public int bringUpSeriesData(String configFileName) {
       ...
       // 	iSeriesLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        ...
        System.out.println("Building a NucleiMgr and setting ImageWindowAT useStack, splitMode flags: " + iUseStack + ", " + iSplit);
        NucleiMgr nucMgr = new NucleiMgr(this);
        ImageWindowATstatic.setUseStack(iUseStack);
        ImageWindowATstatic.setSplitMode(iSplit);
        System.out.println("ImageWindowATstatic stack set: "+ iUseStack);
        System.out.println("ImageWindowATstatic split mode: " + iSplit);
    ...
    public int bringUpSeriesData(Config config) {
        ...
        iImgWin.setUseStack(iUseStack);
        iImgWin.setSplitMode(iSplit);
        System.out.println("ImageWindowAT static stack set: "+ iUseStack);
        System.out.println("ImageWindowAT static stack set: " + iSplit);
        if (!nucMgr.iGoodNucleiMgr) {
            return -1;
        }
        //nucMgr.processNuclei(false, nucMgr.getConfig().iNamingMethod);
    ...
    public int bringUpSeriesData(ImagePlus imp, String nucZipPath) {
        System.out.println("bringUpSeriesData: " + nucZipPath);
        File fx = new File(nucZipPath);
//        String ss = TITLE + ": " + fx.getName();
//        iMainFrame.setTitle(ss);

        // this is the only place where we construct a NucleiMgr
        NucleiMgr nucMgr = new NucleiMgr(this, imp, nucZipPath);

        if (!nucMgr.iGoodNucleiMgr) {
            return -1;
        }
        //nucMgr.processNuclei(false, nucMgr.getConfig().iNamingMethod);
        nucMgr.setConfig(new Config());
        nucMgr.getConfig().iNamingMethod = Identity3.NEWCANONICAL;
        nucMgr.processNuclei(true, Identity3.NEWCANONICAL);
        String configName = Config.getShortName(nucZipPath);
        nucMgr.getConfig().iConfigFileName= configName;
        if (!iNucleiMgrHash.containsKey(configName)) {
            iNucleiMgrHash.put(configName, nucMgr);
            if(fullGUI)
            	iAceMenuBar.addToRecent(configName);
...
//        setShowAnnotations(iShowAnnotationsSave);
    ...
    */
    ...
    public void restoreTree(String shortName) {
        ...
        iImgWin.setNucleiMgr(iNucleiMgr);
        iImgWin.setStaticParameters(iNucleiMgr.getConfig().iZipTifFilePath,
...
//        setShowAnnotations(false);
    ...
    public void setStartingCell(Cell c, int time) {
		...
		if(iImgWin!=null){
			iImgWin.setAceTree(this);
			if (imagePlusSourceOfImages)
				iImgWin.refreshDisplay(sourceImp,sourceImp.getTitle());
			else
				iImgWin.refreshDisplay(makeImageName());
		}
    }
    ...
    private void setKeyboardActions() {
        ...
        Action up = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
                ...
                incPlane(-1);
                iTrackPosition = ImageWindowAT.NONE;
                updateDisplay();
            }
        };
        getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(s), s);
...
//    }
	...
	public void handleImage() {
        ...
        if (cfile == null) {
        	...
        	//System.out.println("AceTree calling iImgWin.makeImage(null)...");
        	iImgWin.makeImage(null);
            return;
        }
        if (iImgWin == null){
        	if (imagePlusSourceOfImages){
        		iImgWin = new ImageWindowAT(iTifPrefix + cfile, null, iPlayerControl);
        	}else{
        		iImgWin = new ImageWindowAT(iTifPrefix + cfile, null, iPlayerControl);
        		iImgWin.setUseStack(iUseStack);
        		iImgWin.setSplitMode(iSplit);
            ...
            if (iImgWin != null) {
                try {
                	iImgWin.imagewindowPlaneNumber = iImagePlane;
                	iImgWin.iImagePlane = iImagePlane;
                	iImgWin.iPlaneInc = iPlaneInc;
                	iImgWin.iImageTime = iImageTime;
                	if(!imagePlusSourceOfImages)
                		ip = iImgWin.refreshDisplay(iTifPrefix + cfile);
                	else {
                		ip = iImgWin.refreshDisplay(sourceImp, cfile);
                	}
                	} catch(Exception e) {
				...
				// ip = iImgWin.makeImage(iTifPrefix + cfile);
                //System.out.println("AceTree calling iImgWin.makeImage2...");
				ip = iImgWin.makeImage2(iTifPrefix + cfile, getImagePlane(), getUseStack(), getiSplit());
				 //iImgWin = new ImageWindowAT( cfile, ip);
				try {
					iImgWin = new ImageWindowAT(iTifPrefix + cfile, ip, iPlayerControl);
					//System.out.println("AceTree passing to ImageWindowAT title: "+iTifPrefix + cfile);
	                iImgWin.setAceTree(this);
    ...
    public String makeImageName()
    {
    	if (!imagePlusSourceOfImages) {			
			...
			return imageNameHandler(iImageTime, iImagePlane);
		}
    }
    ...
    private int trackCellPlane() {
        if (iTrackPosition != iImgWin.NONE) {
            iPlaneInc = 0;
...
//    }
    ...
    private String makeDisplayText() {
        ...
        if (iCurrentCellPresent) {
            ...
            //if (iTrackPosition != iImgWin.NONE) track = "\ntracking";
            sb2.append(track);
            }
        } else
...
////////////////////////////////////////////////////////////////////
    ...
    private void copyImage() {
        ...
        ImageWindowAT iw = new ImageWindowAT(s, ip,iPlayerControl);
...
*/
    ...
    public synchronized void bookmarkTool() {
    	...
    	if (iBookmarkDialog != null) {
    		...
    		// Pass bookmarked list of cells to ImageWindowAT for rendering in another color
    		if (iImgWin != null)
    			iImgWin.setBookmarkList(iBookmarkJList.getModel());
...
// introduced to permit right click on tree to select end time of cell
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().equals(NEXTT)) {
            ...
            if (iCurrentCell.isAnterior()) iTrackPosition = ImageWindowAT.ANTERIOR;
            else iTrackPosition = ImageWindowAT.POSTERIOR;
            //setTrack();
        }
    ...
    public void forceTrackingOn() {
        iTrackPosition = ImageWindowAT.ANTERIOR;
        iTrackPositionSave = ImageWindowAT.POSTERIOR;
    }
...
//    }
	...
	private void doDaughterDisplayWork(Cell parent, Cell selectedDaughter) {
        ...
        if (selectedDaughter != null) {
            if (selectedDaughter == anteriorCell) iTrackPosition = ImageWindowAT.ANTERIOR;
            else iTrackPosition = ImageWindowAT.POSTERIOR;
        }
        Cell save = iCurrentCell;
        if (iTrackPosition == iImgWin.ANTERIOR) 
    ...
    public void setCurrentCell(Cell c, int time, int source) {
	    ...
	    if(iImgWin != null)
        	...
        	Cell old = iCurrentCell;
        	iCurrentCell = c;
        	iImagePlane = c.getPlane();
		    if(iImgWin != null)
		    	iImgWin.updateCurrentCellAnnotation(iCurrentCell, old, time);
            showSelectedCell(c, time);
	...
	private void showSelectedCell(Cell c, int requestedTime) {
        ...
        if (c == null ) {
        	...
        	return;
        } else if(imagePlusSourceOfImages){
            iImageTime = requestedTime;
            iTimeInc = 0;
            iImagePlane = c.getPlane();
            iPlaneInc = 0;
    ...
    */
    ...
    /**
     * called from ImageWindowAT when user clicks a nucleus
    ...
    public void killCell(int x) {
		...
		if(c!=null){
			this.setStartingCell(c, currenttime);
    ...
    */
    ...
    public void updateUseZip(int useZip) {
    	...
    	iImgWin.cUseZip = useZip;
    	iNucleiMgr.getConfig().iUseZip = useZip;
    }

    public ImageWindowAT getiImgWin() {
		return iImgWin;
	}

	public void setiImgWin(ImageWindowAT iImgWin) {
		this.iImgWin = iImgWin;
	}
...
*/
    ...
    public boolean isTracking() {
        //System.out.println("isTracking: " + iTrackPosition + CS + (iTrackPosition != iImgWin.NONE));
        return iTrackPosition != ImageWindowAT.NONE;
    }
    ...
    public ImageWindowAT getImageWindowAT() {
    	//System.out.println("ImageWindowAT requested.");
        return iImgWin;
    }
    
    public void showImageWindowAT() {
    	if (iImgWin != null)
