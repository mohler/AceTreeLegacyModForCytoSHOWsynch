...
public class NucleiMgr {
    ...
    */
	...
	public NucleiMgr(AceTree aceTree) {
        ...
        iAceTree = aceTree;
        String configFileName = aceTree.getConfigFileName();
        iImgWin = iAceTree.getiImgWin();
    ...
    public NucleiMgr(AceTree aceTree, ImagePlus imp, String nucZipPath) {
    	System.out.println("Creating NucleiMgr using metadata from CytoSHOW ImagePlus");
    	...
    	iStartingIndex = 1;
    	iEndingIndex = imp.getNFrames();
    	iStartTime = 1;
    	iLastNucleiFile = 0;
    	iMeasureCSV = new MeasureCSV(nucZipPath);
//    	iConfig = new Config(nucZipPath, false);

    	iZipNuclei = new ZipNuclei(nucZipPath);
    	if (iZipNuclei.iZipFile != null) {
    		long timeStart = System.nanoTime();
    		readNuclei(imp);
    		long timeEnd = System.nanoTime();
    		...
    		iPlaneEnd = imp.getNSlices();
    		iPlaneStart = 1;
    		iZPixRes = imp.getCalibration().pixelDepth/imp.getCalibration().pixelWidth;

            NucUtils.setZPixRes(iZPixRes);
    		iGoodNucleiMgr = true;
    		setParameterEntry(new File(nucZipPath).getParent());
            iImageWidth = imp.getWidth();
            iImageHeight = imp.getHeight();
    	}
...
//    	computeRWeights();
	...
	public NucleiMgr(String nucZipPath) {
        iZipNuclei = new ZipNuclei(nucZipPath);
        if (iZipNuclei.iZipFile != null) {
            //20060719 readEditLog(iEditLog);
            long timeStart = System.nanoTime();
            readNucleiwithoutdickingaround();
            long timeEnd = System.nanoTime();
            double timeDiff = (timeEnd-timeStart)/1e6;
            System.out.println("Time to read nuclei in constructor(Config c): "+timeDiff+" ms.");
            iGoodNucleiMgr = true;
    ...
    private void findImageParameters() {
        // save off existing ImageWindowAT parameters
        String zipTifFilePath = ImageWindowATstatic.cZipTifFilePath;
        String tifPrefix = ImageWindowATstatic.cTifPrefix;
        String tifPrefixR = ImageWindowATstatic.cTifPrefixR;
        int useZip = ImageWindowATstatic.cUseZip;
        int width = ImageWindowATstatic.cImageHeight;
        int height = ImageWindowATstatic.cImageWidth;
        // ***************************************

        // now feed it my parameters
        sendStaticParametersToImageWindowAT();
        ...
        // now "make" the image
        System.out.println("\nNucleiMgr calling ImageWindowATstatic.makeImage on image: " + getConfig().iTifPrefix + imageName);
        ImageWindowATstatic.makeImage(getConfig().iTifPrefix + imageName);
        iImageWidth = ImageWindowATstatic.cImageWidth;
        iImageHeight = ImageWindowATstatic.cImageHeight;
        // ********************************


        // now restore ImageWindow
        ImageWindowATstatic.cZipTifFilePath = zipTifFilePath;
        ImageWindowATstatic.cTifPrefix = tifPrefix;
        ImageWindowATstatic.cTifPrefixR = tifPrefixR;
        ImageWindowATstatic.cUseZip = useZip;
        ImageWindowATstatic.cImageWidth = width;
        ImageWindowATstatic.cImageHeight = height;
        ImageWindowATstatic.imagewindowUseStack = iUseStack;
        ImageWindowATstatic.iSplit = iSplit;
    }
...
//    }
    ...
    private void readNuclei(ImagePlus imp) {
        int last = readNuclei(iZipNuclei);
   	}

    public void readNucleiwithoutdickingaround() {
        int last = readNuclei(iZipNuclei);
    }
...
//        System.out.println("readNuclei:3 " + iMovie.time_end + CS + iMovie.time_start);
    ...
    public void sendStaticParametersToImageWindowAT() {
        ImageWindowATstatic.setStaticParameters(
    ...
    public Vector<Nucleus> getParameterFileInfo() {
        ...
        if (iParameters!=null)
        	iParameters.getParameterFileInfo();
